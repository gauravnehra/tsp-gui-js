(function (graph) {
    ...
    // graph.extendAlgorithm(Dynamic, 'BranchNBound', 'initCachedDistances');
    // graph.extendAlgorithm(Dynamic, 'BranchNBound', 'lookupDistance');
    ...
    Dynamic.prototype.initCachedDistances = function() {
        this.cachedDistances = [];
        var pts = this.solver.points;
        for(var i = 0; i < pts.length; i++) {
            this.cachedDistances[i] = [];
            for (var j = i + 1; j < pts.length; j++) {
                this.cachedDistances[i][j - i - 1] = graph.distance(pts[i],pts[j]);
            }
        }
    };

    Dynamic.prototype.lookupDistance = function (index1, index2) {
